...
{
    inner: ComboboxCore<'a, DisplayValue<T>, Message, Renderer>,
}
    ...
    Renderer: core_text::Renderer + 'a,
{
    ...
    /// Sets the message that will be produced when the combobox opens.
    pub fn on_open(mut self, message: Message) -> Self {
        self.inner = self.inner.on_open(message);
        self
    }

    /// Sets the message that will be produced when the combobox closes.
    pub fn on_close(mut self, message: Message) -> Self {
        self.inner = self.inner.on_close(message);
        self
    }

    /// Sets the floating label of the combobox.
...
where
    ...
    Renderer: core_text::Renderer + 'a,
{
    fn from(combobox: Combobox<'a, T, Message, Renderer>) -> Self {
        Element::new(combobox.inner)
    }
}
    ...
    Renderer: core_text::Renderer + 'a,
{
    ...
    let inner = ComboboxCore::new(state.inner(), placeholder, display_value.as_ref(), {
...
struct ComboboxCore<'a, T, Message, Renderer>
where
    T: Display + Clone,
    Renderer: core_text::Renderer,
{
...
impl<T, Message, Renderer> fmt::Debug for ComboboxCore<'_, T, Message, Renderer>
where
    T: Display + Clone,
    Renderer: core_text::Renderer,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ComboboxCore").finish_non_exhaustive()
    }
}

impl<'a, T, Message, Renderer> ComboboxCore<'a, T, Message, Renderer>
where
    T: Display + Clone,
    Renderer: core_text::Renderer,
{
...
impl<T, Message, Renderer> Widget<Message, Theme, Renderer>
    for ComboboxCore<'_, T, Message, Renderer>
where
...
impl<'a, T, Message, Renderer> From<ComboboxCore<'a, T, Message, Renderer>>
    for Element<'a, Message, Theme, Renderer>
where
    T: Display + Clone + 'static,
    Message: Clone + 'a,
